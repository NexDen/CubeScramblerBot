...
function getLLScramble(type, length, cases) {
    ...
    if (type == '222tcp') {
        ...
        llcase = egll_map[scrMgr.fixCase(cases, egllprobs)];
        perm = perm.concat(egperms[2 + mathlib.rn(4)]);
    } else if (type == '222eg2') {
        llcase = egll_map[scrMgr.fixCase(cases, egllprobs)];
...
function getScramble(type, length, state) {
    ...
    do {
        ...
        if (type == '222o' || type == '222so') {
            ...
            var arr = mathlib.setNPerm([0, 0, 0, 0].concat(egperms[perm]), mathlib.rn(24), 4);
...
module.exports = {
    // Random-state 2x2x2 scramble. Defaults to "222so" (full random state).
    getScramble: (type) => getScramble(type || '222so')
};
